# Sunny API Files Migration Script
# PowerShell script to migrate files to the new API structure

Write-Host "🚀 Starting Sunny API Files Migration..." -ForegroundColor Green
...
foreach ($source in $migrations.Keys) {
    ...
    if (Test-Path $source) {
        try {
            ...
            Write-Host "❌ Failed to copy $source : $($_.Exception.Message)" -ForegroundColor Red
            $errorCount++
        }
    }
    else {
...
Write-Host "📁 Total directories created: $($apiDirs.Count + $packageDirs.Count)" -ForegroundColor Yellow
